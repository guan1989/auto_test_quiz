...
class DDT:
    ...
    def __run_test(self,allure_dir):
        ...
        pytest.main(['-s', path + f'ddt\\test_{self.type}_{self.suite_idx + 1}.py', '--alluredir', allure_dir ])
        # 跑完后，suite_idx + 1
        self.suite_idx += 1
    ...
    def run_app_cases(self, case_path='APP自动化示例.xlsx'):
        """
        读取用例为三维列表，并执行
        :param case_path: 用例路径
        """
        ...
        # 把文件名改回去
        self.writer.save_close()
        os.rename(path + f'ddt\\test_{self.type}_{self.suite_idx}.py', 'ddt\\test_app_0.py')
